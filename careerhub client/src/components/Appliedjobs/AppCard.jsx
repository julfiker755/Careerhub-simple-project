...
const AppCard = ({appyjobsall}) => {
    const {id,logo,job_title,company_name,job_type,remote_or_onsite}=appyjobsall
    const navigate=useNavigate()
    
    return (
        <div className='border rounded-md p-4 flex  justify-between items-center'>
        <div className='flex items-center gap-4'>
          <div className='bg-[#82131eee] rounded-md p-10'> <img className='h-fit' src={logo} alt="" /></div>
         <div>
        ...
        <button  onClick={()=>navigate(`/single/${id}`)} className='bg-[#209558] h-fit py-2 text-white rounded-md px-4'>View Details</button>
    </div>
    );
};
